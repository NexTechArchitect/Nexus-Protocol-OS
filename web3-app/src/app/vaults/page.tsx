...
function useVaultHistory(userAddress: `0x${string}` | undefined) {
    ...
    try {
      ...
      const [addedLogs, removedLogs] = await Promise.all([
        publicClient.getLogs({
          ...
          event: parseAbiItem('event LiquidityAdded(address indexed provider, uint256 assetsAdded, uint256 sharesMinted)'),
          args: { provider: userAddress },
          fromBlock: from, toBlock: latest,
        }),
        publicClient.getLogs({
          address: CONTRACTS.VAULT.address,
          event: parseAbiItem('event LiquidityRemoved(address indexed provider, uint256 assetsRemoved, uint256 sharesBurned)'),
          args: { provider: userAddress },
          fromBlock: from, toBlock: latest,
        }),
      ]);
      ...
      for (const log of addedLogs) {
        const { assetsAdded, sharesMinted } = log.args as any;
        items.push({ hash: log.transactionHash ?? '0x', type: 'ADD_LIQUIDITY', usdcAmount: Number(formatUnits(assetsAdded as bigint, 18)).toFixed(2), shares: Number(formatUnits(sharesMinted as bigint, 18)).toFixed(4), blockNumber: log.blockNumber ?? BigInt(0) });
      }
      for (const log of removedLogs) {
        const { assetsRemoved, sharesBurned } = log.args as any;
        items.push({ hash: log.transactionHash ?? '0x', type: 'REMOVE_LIQUIDITY', usdcAmount: Number(formatUnits(assetsRemoved as bigint, 18)).toFixed(2), shares: Number(formatUnits(sharesBurned as bigint, 18)).toFixed(4), blockNumber: log.blockNumber ?? BigInt(0) });
      }
...
const Toast = ({ title, msg, type, hash, onClose }: any) => {
  ...
  return (
    <div onClick={() => hash?.startsWith('0x') && window.open(`https://sepolia.etherscan.io/tx/${hash}`, '_blank')}
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody>
                      {history.map((item,idx)=>(
                        <tr key={idx} onClick={()=>window.open(`https://sepolia.etherscan.io/tx/${item.hash}`,'_blank')} className="hover:bg-[#FFFBEB]/40 transition-colors cursor-pointer group border-b border-slate-50 last:border-0">
          ...
          <div className="h-fit sticky top-28">
            ...
            <div className="bg-white border-x border-b border-slate-100 rounded-b-2xl shadow-[0_8px_40px_rgba(240,185,11,0.12)] p-8">
              ...
              <div className="mt-7 pt-6 border-t border-slate-100 space-y-3">
                {[{label:'Asset',val:'USDC (Sepolia)',icon:true},{label:'Strategy',val:'Delta Neutral LP'},{label:'Lock-up',val:'None (Liquid)',green:true},{label:'Share Token',val:'HLV'},{label:'Your Shares',val:`${formatted.userShares} HLV`}].map((r)=>(
