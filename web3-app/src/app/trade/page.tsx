...
function decodeLockedCollateral(raw: bigint): number {
  if (!raw || raw === BigInt(0)) return 0;
  // getLockedCollateral() returns 18-dec (lockedCollateral mapping stores 18-dec)
  const v = parseFloat(formatUnits(raw, 18));
  if (v >= 0 && v <= 100_000_000) return v;
  return 0;
}
...
export default function TradePage() {
  ...
  useEffect(() => { btcPriceRef.current = btcLive.price; }, [btcLive.price]);
  useEffect(() => { ethPriceRef.current = ethLive.price; }, [ethLive.price]);
...
useEffect(() => {
    ...
    const tryRefetch = () => {
      ...
      if (attempts < MAX_ATTEMPTS) {
        ...
        setTimeout(tryRefetch, delay);
      }
    };
    setTimeout(tryRefetch, 1500);
// eslint-disable-next-line react-hooks/exhaustive-deps
}, [closeConfirmed]);
  // ── Derived Values ────────────────────────────────────────────────────────
...
const handleMarketClose = useCallback(async (closeAsset: Asset) => {
    ...
    try {
      const livePrice = closeAsset.symbol === "BTC"
        ? btcPriceRef.current
        : ethPriceRef.current;
      ...
      const hash = await writeContractAsync({
        ...
        args: [closeAsset.address, parseUnits(livePrice.toFixed(18), 18)],
      });
