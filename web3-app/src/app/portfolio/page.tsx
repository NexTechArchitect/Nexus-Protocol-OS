...
const ASSET_LABELS: Record<string, string> = {
  '0x29f2d40b0605204364af54ec677bd022da425d03': 'BTC',
  '0xfff9976782d46cc05630d1f6ebab18b2324d6b14': 'ETH',
};

// ─── useOnChainHistory hook ───────────────────────────────────────────────────
function useOnChainHistory(userAddress: `0x${string}` | undefined) {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Fetch last ~50000 blocks (Sepolia ~7 days)
      ...
      const openedLogs = await publicClient.getLogs({
        ...
        event: parseAbiItem('event PositionOpened(address indexed user, address indexed asset, bool isLong, uint256 collateral, uint256 leverage, uint256 entryPrice, uint8 mode)'),
      ...
      const closedLogs = await publicClient.getLogs({
        address: CONTRACTS.POSITION_MANAGER.address,
        event: parseAbiItem('event PositionClosed(address indexed user, address indexed asset, int256 pnl, uint256 payout)'),
        args: { user: userAddress },
        fromBlock, toBlock: latestBlock,
      });

      // Deposited events (vault)
      const depositedLogs = await publicClient.getLogs({
        address: CONTRACTS.VAULT.address,
        event: parseAbiItem('event Deposited(address indexed user, uint256 rawAmount, uint256 scaledAmount)'),
        args: { user: userAddress },
        fromBlock, toBlock: latestBlock,
      });

      // Withdrawn events (vault)
      const withdrawnLogs = await publicClient.getLogs({
        address: CONTRACTS.VAULT.address,
        event: parseAbiItem('event Withdrawn(address indexed user, uint256 rawAmount, uint256 scaledAmount)'),
        args: { user: userAddress },
        fromBlock, toBlock: latestBlock,
      });
      ...
      for (const log of openedLogs) {
        const { asset, isLong, collateral, leverage } = log.args as any;
        const assetLabel = ASSET_LABELS[asset?.toLowerCase()] ?? asset?.slice(0, 6);
        const collat = collateral >= BigInt('1000000000000000')
          ? Number(formatUnits(collateral, 18))
          : Number(formatUnits(collateral, 6));
        const lev = leverage >= BigInt('1000000000000000000')
          ? Number(leverage / BigInt('1000000000000000000'))
          : Number(leverage);
        items.push({
          hash: log.transactionHash ?? '0x',
          type: 'TRADE_OPEN',
          asset: assetLabel,
          amount: collat.toFixed(2),
      ...
      for (const log of closedLogs) {
        const { asset, pnl, payout } = log.args as any;
        const assetLabel = ASSET_LABELS[asset?.toLowerCase()] ?? asset?.slice(0, 6);
        const pnlNum = Number(formatUnits(pnl as bigint, 18));
        const payoutNum = payout >= BigInt('1000000000000000')
          ? Number(formatUnits(payout as bigint, 18))
          : Number(formatUnits(payout as bigint, 6));
        items.push({
          hash: log.transactionHash ?? '0x',
          type: 'TRADE_CLOSE',
          asset: assetLabel,
          amount: payoutNum.toFixed(2),
          pnl: Math.abs(pnlNum).toFixed(4),
      ...
      for (const log of depositedLogs) {
        const { rawAmount } = log.args as any;
        items.push({
          hash: log.transactionHash ?? '0x',
          type: 'DEPOSIT',
          amount: Number(formatUnits(rawAmount as bigint, 6)).toFixed(2),
          timestamp: 0,
          blockNumber: log.blockNumber ?? BigInt(0),
        });
      }
...
const Toast = ({ title, msg, type, hash, onClose }: any) => {
  ...
  return (
    <div
      onClick={() => hash && window.open(`https://sepolia.etherscan.io/tx/${hash}`, '_blank')}
      className="fixed bottom-6 right-6 z-[200] w-[90%] max-w-sm cursor-pointer group"
      style={{ animation: 'slideUp 0.4s cubic-bezier(0.34,1.56,0.64,1)' }}
    >
      <div className={`px-5 py-4 rounded-2xl shadow-2xl border bg-white flex items-start gap-4 hover:-translate-y-1 transition-transform ${
        type === 'success' ? 'border-l-4 border-l-[#F0B90B] border-t-slate-100 border-r-slate-100 border-b-slate-100' :
        type === 'loading' ? 'border-l-4 border-l-blue-500 border-slate-100' :
...
const MetricCard = ({ label, val, sub, accent = false }: { label: string; val: string; sub: string; accent?: boolean }) => (
  <div className={`rounded-2xl p-6 border transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${
    accent ? 'bg-[#F0B90B] border-[#D9A10A] shadow-[0_4px_24px_rgba(240,185,11,0.25)]' : 'bg-white border-slate-100 shadow-sm'
  }`}>
  ...
  <tr className="hover:bg-[#FFFBEB]/60 transition-colors group border-b border-slate-50 last:border-0">
    <td className="px-8 py-6">
      <div className="flex items-center gap-4">
        <div
          className="w-11 h-11 rounded-2xl flex items-center justify-center font-black text-lg shadow-sm group-hover:scale-110 transition-transform"
          style={{ background: pos.color + '18', border: `1.5px solid ${pos.color}40`, color: pos.color }}
        >
          {pos.icon}
        </div>
        <div>
    ...
    <td className="px-8 py-6">
      <span className={`text-[10px] font-black px-3 py-1.5 rounded-lg uppercase tracking-wider ${
        pos.isLong ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
      }`}>
    ...
    <td className="px-8 py-6">
      <div className={`font-mono font-black ${pos.pnl.isPositive ? 'text-emerald-600' : 'text-red-500'}`}>
        {pos.pnl.isPositive ? '+' : '-'}${pos.pnl.value}
      </div>
      <div className={`text-[10px] font-bold mt-0.5 ${pos.pnl.isPositive ? 'text-emerald-500' : 'text-red-400'}`}>
        {pos.pnl.isPositive ? '+' : '-'}{pos.pnl.percent}% ROE
      </div>
    </td>
    ...
    <td className="px-8 py-6">
      <button
        disabled={isClosing}
        onClick={() => onClose(pos.assetAddress)}
...
export default function PortfolioPage() {
  ...
  const { history: onChainHistory, loading: historyLoading, refetch: refetchHistory } = useOnChainHistory(address);

  const [notify, setNotify] = useState<{ title: string; msg: string; type: 'loading' | 'success' | 'error'; hash?: string } | null>(null);
  const [sessionHistory, setSessionHistory] = useState<HistoryItem[]>([]);
  const [activeTab, setActiveTab] = useState<'BALANCES' | 'POSITIONS' | 'HISTORY'>('POSITIONS');
  ...
  const { handleDeposit, handleWithdraw, getActionState, currentHash, isLoading: vaultLoading, status } = useVaultOperations(
    (t, m, type, hash) => setNotify({ title: t, msg: m, type, hash })
  );
  ...
  useEffect(() => {
    if (closeSuccess) {
      ...
      setTimeout(() => { void refetch(); void refetchHistory(); }, 1500);
      setTimeout(() => { void refetch(); void refetchHistory(); }, 5000);
    }
  }, [closeSuccess]);
  ...
  const handleClosePosition = useCallback(async (assetAddress: `0x${string}`) => {
    ...
    try {
      ...
      setNotify({ title: 'Close Failed', msg: e?.shortMessage ?? 'Transaction rejected', type: 'error' });
      setClosingAsset(null);
    }
  }, [writeContractAsync]);

  // Auto close action panel on success
  useEffect(() => {
    if (status === 'SUCCESS' && actionMode && amount) {
      setSessionHistory(prev => [{ hash: currentHash ?? '0x...', type: actionMode, amount, timestamp: Date.now(), blockNumber: BigInt(0) }, ...prev]);
      setTimeout(() => void refetchHistory(), 2000);
      const t = setTimeout(() => { setActionMode(null); setAmount(''); }, 2500);
      return () => clearTimeout(t);
    }
  }, [status]);
  ...
  const buttonText = () => {
    ...
    if (actionMode === 'DEPOSIT') return getActionState(amount) === 'NEEDS_APPROVAL' ? 'Step 1: Approve USDC' : 'Step 2: Confirm Deposit';
  ...
  return (
    <div className="relative w-full min-h-screen pb-24 bg-[#FAFAF7] text-[#0f172a]">
      <style>{`
        ...
        .stagger-1 { animation: fadeIn 0.5s ease both; }
        .stagger-2 { animation: fadeIn 0.5s ease 0.1s both; }
        .stagger-3 { animation: fadeIn 0.5s ease 0.2s both; }
        .stagger-4 { animation: fadeIn 0.5s ease 0.3s both; }
      ...
      <main className="max-w-[90rem] mx-auto px-6 pt-28 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 mb-12 stagger-1">
          ...
          <div className="flex gap-3 w-full lg:w-auto stagger-2">
            <button onClick={() => setActionMode('DEPOSIT')} className="flex-1 lg:flex-none px-8 py-4 bg-[#F0B90B] text-white font-black rounded-2xl shadow-lg hover:shadow-xl hover:bg-[#D9A10A] hover:scale-[1.02] active:scale-95 transition-all text-sm tracking-widest uppercase">
              Deposit
            </button>
            <button onClick={() => setActionMode('WITHDRAW')} className="flex-1 lg:flex-none px-8 py-4 bg-white text-slate-900 border border-slate-200 font-black rounded-2xl shadow-sm hover:bg-slate-50 active:scale-95 transition-all text-sm tracking-widest uppercase">
              Withdraw
            </button>
            <button onClick={refetch} className="px-4 py-4 bg-white border border-slate-200 rounded-2xl shadow-sm hover:bg-slate-50 transition-all text-slate-500 font-bold" title="Refresh">
              ↻
            </button>
          </div>
        </div>
        ...
        {actionMode && (
          <div className="mb-12 bg-white/95 border border-[#F0B90B]/20 rounded-3xl p-8 shadow-2xl stagger-2" style={{ animation: 'fadeIn 0.3s ease' }}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setActionMode(null)} className="w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 text-slate-400 hover:bg-slate-200 transition-all font-bold">✕</button>
            </div>
            <div className="flex flex-col lg:flex-row gap-6">
              <div className="flex-1">
                <div className="flex justify-between mb-3 px-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Amount (USDC)</label>
                  <span className="text-[10px] font-bold text-[#92600A] bg-[#F0B90B]/10 px-3 py-1 rounded-lg">
                    Available: {actionMode === 'DEPOSIT' ? metrics.walletBalance : metrics.freeCollateral}
                  </span>
                </div>
              ...
              <div className="flex items-end">
                <button
                  onClick={() => actionMode === 'DEPOSIT' ? handleDeposit(amount) : handleWithdraw(amount)}
                  disabled={vaultLoading || !amount || parseFloat(amount) <= 0}
                  className={`h-[84px] w-full lg:w-[240px] font-black rounded-2xl text-base shadow-lg transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-3 uppercase tracking-widest ${
                    status === 'SUCCESS' ? 'bg-emerald-500 text-white' :
                    actionMode === 'DEPOSIT' && getActionState(amount) === 'NEEDS_APPROVAL' ? 'bg-amber-500 text-white hover:bg-amber-600' :
                    'bg-[#F0B90B] text-white hover:bg-[#D9A10A]'
                  }`}
                >
            ...
            <div className="mt-5 flex items-start gap-3 p-4 rounded-xl bg-slate-50 border border-slate-100">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                {actionMode === 'DEPOSIT'
                  ? 'Deposited funds go into your Nexus Vault and can be used as margin for trades. USDC approval required on first deposit.'
                  : 'Only free (unlocked) collateral can be withdrawn. Collateral in active positions must be freed by closing positions first.'}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-slate-100 rounded-3xl overflow-hidden shadow-lg stagger-4">
          <div className="flex border-b border-slate-100 px-8 pt-6 gap-8 overflow-x-auto">
            {(['POSITIONS', 'BALANCES', 'HISTORY'] as const).map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`pb-4 text-[10px] font-black tracking-[0.2em] transition-all relative whitespace-nowrap uppercase ${
                  activeTab === tab ? 'text-[#92600A]' : 'text-slate-400 hover:text-slate-600'
                }`}
              >
                {tab}
                {tab === 'POSITIONS' && hasPositions && (
                  <span className="ml-2 text-[9px] bg-[#F0B90B] text-white px-1.5 py-0.5 rounded-full font-black">{positions.length}</span>
                )}
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-slate-50/70 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                      <tr>
                        ...
                        <th className="px-8 py-5">Mark</th>
                        <th className="px-8 py-5">PnL</th>
                        <th className="px-8 py-5">Liq. Price</th>
                    ...
                    <tbody>
                      {positions.map((pos) => (
                        <PositionRow
                          ...
                          isClosing={closingAsset === pos.assetAddress}
                        />
                      ))}
                    </tbody>
                  </table>
              ...
              <table className="w-full text-left min-w-[600px]">
                <thead className="bg-slate-50/70 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-8 py-5 text-right">Total Balance</th>
                    <th className="px-8 py-5 text-right">Free Margin</th>
                    <th className="px-8 py-5 text-right">In Positions</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="hover:bg-[#FFFBEB]/40 transition-colors border-b border-slate-50">
                    <td className="px-8 py-7 flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center font-black text-sm shadow-md">U</div>
                      <div>
                        <span className="block font-black text-slate-900 text-base">USDC</span>
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Sepolia Stablecoin</span>
                      </div>
                    </td>
                  ...
                  <tr className="hover:bg-[#FFFBEB]/40 transition-colors">
                    <td className="px-8 py-5 flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-600 flex items-center justify-center font-black text-sm shadow-sm">💼</div>
                      <div>
                        <span className="block font-black text-slate-700 text-base">Wallet USDC</span>
                        <span className="text-[10px] text-slate-400 uppercase font-bold">MetaMask Balance</span>
                      </div>
                    </td>
              ...
              <div className="min-w-[700px]">
                {historyLoading ? (
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-slate-50/70 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                      <tr>
                        ...
                        <th className="px-8 py-5 text-right">Tx</th>
                      </tr>
                    </thead>
                    <tbody>
                      {onChainHistory.map((item, idx) => (
                        <tr key={idx}
                          onClick={() => window.open(`https://sepolia.etherscan.io/tx/${item.hash}`, '_blank')}
                          className="hover:bg-[#FFFBEB]/40 transition-colors cursor-pointer group border-b border-slate-50 last:border-0"
                        >
                          <td className="px-8 py-5">
                            <div className="flex items-center gap-2">
                              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${
                                item.type === 'TRADE_OPEN'  ? 'bg-blue-400' :
                                item.type === 'TRADE_CLOSE' ? 'bg-emerald-400' :
                                item.type === 'DEPOSIT'     ? 'bg-[#F0B90B]' :
                                item.type === 'WITHDRAW'    ? 'bg-purple-400' : 'bg-red-400'
                              }`} />
                              <span className="font-black text-slate-800 text-xs uppercase tracking-wide">
                                {item.type === 'TRADE_OPEN'  ? `Open ${item.side ?? ''}` :
                                 item.type === 'TRADE_CLOSE' ? 'Close Trade' :
                                 item.type === 'DEPOSIT'     ? 'Deposit' :
                                 item.type === 'WITHDRAW'    ? 'Withdraw' : 'Liquidated'}
                              </span>
                            </div>
                          </td>
                          <td className="px-8 py-5 font-mono font-bold text-slate-600 text-sm">
                            {item.asset ?? '—'}
                          </td>
                          ...
                          <td className="px-8 py-5 font-mono font-bold text-sm">
                            {item.pnl !== undefined ? (
                              <span className={item.pnlPositive ? 'text-emerald-600' : 'text-red-500'}>
                                {item.pnlPositive ? '+' : '-'}${item.pnl}
                              </span>
                            ) : <span className="text-slate-300">—</span>}
                          </td>
                          <td className="px-8 py-5">
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#F0B90B]/10 text-[#92600A] text-[10px] font-black uppercase tracking-wide border border-[#F0B90B]/20">
                              <span className="w-1.5 h-1.5 rounded-full bg-[#F0B90B]" /> Confirmed
                            </span>
                          </td>
                          <td className="px-8 py-5 text-right font-mono text-xs text-slate-400 group-hover:text-[#92600A] transition-colors">
                            {item.hash.slice(0, 8)}...↗
                          </td>
                        </tr>
                      ))}
                  ...
                  <div className="flex flex-col items-center justify-center h-72 gap-3">
                    ...
                    <p className="font-black text-sm uppercase tracking-widest text-slate-400">No Transaction History</p>
                    <p className="text-xs text-slate-400">Your trades, deposits & withdrawals will appear here</p>
                    <button onClick={refetchHistory} className="mt-2 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-[#92600A] bg-[#F0B90B]/10 hover:bg-[#F0B90B]/20 rounded-xl transition-colors border border-[#F0B90B]/20">
                      Refresh
                    </button>
                  </div>
                )}
